...
# zadania 10
```{r}
# dla przekładu dla marki Toyota
procKlima <- function(Marka = "") {
  if(Marka == "") {
  zad_10 <- auta2012 %>% mutate(klimatyzacja = grepl(pattern = "klimatyzacja", Wyposazenie.dodatkowe)) %>% 
  filter(Marka != "") %>% filter(Marka == "Toyota")
  } 
  
percent <- data.frame(percent_rank(zad_10$klimatyzacja))
...
# zadanie 18 ok - dodanie max prędkości
```{r}
