...
procKlima <- function(Marka = "toyota") {
  if(Marka == "Toyota") {
  zad_10 <- auta2012 %>% 
...
# brak automatycznefo wybierania Marki
# brak liczenia procentow

# trzeba zaprogramowac automatyczne wybieranie wartosci z kolumny Marka i policzyć procent
```
...
zad_18 <- auta2012 %>% filter(Rok.produkcji == "2006",
                                  ...
                                  group_by(Marka) %>%
                                  summarise(srednia = mean(Cena.w.PLN, na.rm = TRUE)) %>%
                                  summarise(najszybszy = max(srednia))
# problem z dodaniem marki     

```

# zadanie 19 prawie ok
```{r}
zad_19 <- diagnoza %>% filter(eduk4_2013 != "") %>% select(plec, eduk4_2013)
table(zad_19)
# Brak usuniecia wartosci 0
```
